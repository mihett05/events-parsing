...
class NotificationDatabaseModel(Base):
    """Модель базы данных для хранения уведомлений.

    Обеспечивает хранение информации о тексте уведомления, получателе,
    связанном событии, дате отправки и текущем статусе. Поддерживает
    различные форматы уведомлений через перечислимый тип.

    Гарантирует уникальность комбинации события, получателя и даты отправки
    через составное ограничение уникальности.
    """
