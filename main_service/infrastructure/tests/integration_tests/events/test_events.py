...
async def test_create_event(async_client: AsyncClient, user_with_token_model_factory, create_event_model_dto_factory):
    ...
    response = await async_client.post("/v1/events/", json=dto.model_dump(by_alias=True), headers=headers)
    assert response.status_code == 200
    result = EventModel(**response.json())
    assert result.title == dto.title
    assert result.type_ == dto.type_
...
async def test_update_event(async_client: AsyncClient, user_with_token_model_factory, update_event_model_dto_factory):
    ...
    headers = {"Authorization": f"Bearer {user.accessToken}"}
    response = await async_client.put("/v1/events/1", json=dto.model_dump(by_alias=True), headers=headers)
    if response.status_code == 200:
        result = EventModel(**response.json())
        assert result.title == dto.title
