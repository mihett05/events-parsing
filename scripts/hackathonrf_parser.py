...
MONTH_NUM = {'января': '01', 'февраля': '02', 'марта': '03', 'апреля': '04',
    ...
    for item in soup.select('div.t776__textwrapper'):
        ...
        try:
            ...
            registration_match = re.search(r'Регистрация:\s*до\s*(.*?)\n',
                                           description)
            end_registration = parse_date(registration_match.group(1).strip(),
                                          '2024')
        except:
            continue
        events.append(
            Event(title=title, start_date=start_date, end_date=end_date,
                  end_registration=end_registration, description=description))
        # print(events[-1])
    return events
